...
_CK_DIR = Path(__file__).resolve().parent
_ROOT = _CK_DIR.parent
_FRONTEND_DIR = _CK_DIR / "frontend"

for p in [str(_ROOT), str(_CK_DIR), str(_FRONTEND_DIR)]:
    if p not in sys.path:
        sys.path.insert(0, p)

try:
    from frontend.app import run_server
except ImportError:
